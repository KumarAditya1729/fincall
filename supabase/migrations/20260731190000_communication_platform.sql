...
CREATE POLICY "Branch managers can view their providers" ON public.comm_providers
    FOR SELECT USING (public.is_branch_manager() AND branch_id = public.get_current_user_branch_id());

CREATE POLICY "Branch managers can manage their providers" ON public.comm_providers
    FOR ALL USING (public.is_branch_manager() AND branch_id = public.get_current_user_branch_id());
...
CREATE POLICY "Users can view templates for their branch" ON public.comm_templates
    FOR SELECT USING (branch_id = public.get_current_user_branch_id());
...
CREATE TRIGGER update_comm_providers_modtime BEFORE UPDATE ON public.comm_providers FOR EACH ROW EXECUTE FUNCTION public.update_modified_column();
CREATE TRIGGER update_comm_templates_modtime BEFORE UPDATE ON public.comm_templates FOR EACH ROW EXECUTE FUNCTION public.update_modified_column();
CREATE TRIGGER update_comm_campaigns_modtime BEFORE UPDATE ON public.comm_campaigns FOR EACH ROW EXECUTE FUNCTION public.update_modified_column();
CREATE TRIGGER update_comm_history_modtime BEFORE UPDATE ON public.comm_history FOR EACH ROW EXECUTE FUNCTION public.update_modified_column();
CREATE TRIGGER update_comm_preferences_modtime BEFORE UPDATE ON public.comm_preferences FOR EACH ROW EXECUTE FUNCTION public.update_modified_column();

-- Audit logging triggers
CREATE TRIGGER audit_comm_providers AFTER INSERT OR UPDATE OR DELETE ON public.comm_providers FOR EACH ROW EXECUTE FUNCTION public.audit_log_changes();
CREATE TRIGGER audit_comm_templates AFTER INSERT OR UPDATE OR DELETE ON public.comm_templates FOR EACH ROW EXECUTE FUNCTION public.audit_log_changes();
CREATE TRIGGER audit_comm_campaigns AFTER INSERT OR UPDATE OR DELETE ON public.comm_campaigns FOR EACH ROW EXECUTE FUNCTION public.audit_log_changes();
